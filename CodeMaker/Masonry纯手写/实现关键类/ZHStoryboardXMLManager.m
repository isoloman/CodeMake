...
+ (NSArray *)getAllViewControllerWithDic:(NSDictionary *)MyDic andXMLHandel:(ReadXML *)xml{
    ...
    [xml getTargetNodeArrWithName:@"tableViewController" withDic:MyDic withArrM:arrM];
    return arrM;
}
...
+ (NSDictionary *)getAllViewWithAllViewControllerArrM:(NSArray *)arrM andXMLHandel:(ReadXML *)xml{
    ...
    for (NSDictionary *dic in arrM) {
        ...
        if ([[dic valueForKey:@"self_Node_Name"] isEqualToString:@"tableViewController"]) {
            [xml getDicArrFormPathArr:@[@"tableView",@"subviews"] withIndex:0 withDic:tempDic addToArrM:allDic];
        }
        else {
